...
<title>员工管理</title>
<link rel="stylesheet" type="text/css" href="<%=basePath%>pages/css/manage-Staff.css" />
...
<body >
	...
	<div class="manage-Monitor-title">
     ...
     <div class = "manage-Monitor-left-menu-title" id="manage-Monitor-left-menu-title-id"> 
        ...
        <form:form class="manage-car-right-work-data-top-seach-form" id="form" commandName="staffinfo" action="staff_all" method="post"> 
			<div class="manage-car-right-work-data-top-seach" style="background:#bfd9f6">
				<input id="pageNow" type="hidden" name="pageNow" value="${page.pageNow}">
   	    	    ...
   	    	    <div class="manage-staff-right-work-data-top-seach-car">
                   
                 	<div class="manage-staff-right-work-data-top-seach-cartype-carNum">
                      <h style=" font-size:14px; width:30%; height:26px;">&nbsp &nbsp姓名:</h>
                      <form:input path="staffname" class="manage-staff-right-work-seach-info1-carName" />
                     
                    </div>  
                    <div class="manage-staff-right-work-data-top-seach-cartype-carType">  
                      <h style=" font-size:14px; width:30%; height:26px;">手机号:</h>
                      <form:input path="tel" class = "manage-staff-right-work-seach-info2-tel" />
                     </div>    
                                    
                 </div>
                 
                 <button class="manage-staff-right-work-seach-btn-class">检&nbsp;&nbsp;索</button>
			</div>
    ...
    <div class="manage-staff-right-work" id="manage-staff-right-work-id"> 
         ...
         <div class="manage-Monitor-right-work" id= "manage-car-right-work-tableid">
                <div class="manage-Monitor-right-work-data">
                    <nav class="manage-Monitor-right-work-navigation-nav" >
                        ...
                        <li >
                        	<a  href="statistical">
                        		<div style="width:30%;height:55%; margin-top:4%;margin-left:35%;background:url(./pages/img/nav-bg.png);background-position:160px 5px;"></div>
		...
		<form:form id="form_batch_operation" method="post">
            <div class="manage-staff-work-table">
 				<a class="manage-staff-work-btn-add-staff" href ="<%=basePath%>staff_add_page" target="_blank">+新增员工</a>
            	<table class="table table-bordered" >
                    ...
                    <tbody>
                    <c:forEach items="${page.list}" var="staffinfo"> 
                    	<tr>
                        	<th>
	                            <input type="checkbox" name="checkId" value="${staffinfo.id}"></input>
                            </th>
                            <td>${staffinfo.staffname}</td>
                            <td>${staffinfo.sectorinfo.sectorname}</td>
                            <td>${staffinfo.sex==0?'女':'男'}</td>
                            <td>${staffinfo.tel}</td>
                            <td>${staffinfo.stafftype!=0?'是':'否'}</td>
                            <td>${staffinfo.drivertype}</td>
                           
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
                <a class="manage-car-right-work-data-table-del-car" href ="<%= basePath%>staff_delete" target="_blank" onclick="batch_operation(this.href)">删除</a><br/>
            </div>	           
...
var work_layout =	document.getElementById('manage-staff-right-work-id');
